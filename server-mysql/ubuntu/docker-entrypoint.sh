...
check_variables_mysql() {
    ...
    if [ "${MYSQL_ALLOW_EMPTY_PASSWORD,,}" == "true" ] || [ -n "${MYSQL_ROOT_PASSWORD}" ]; then
        ...
        DB_SERVER_ROOT_PASS=${MYSQL_ROOT_PASSWORD:-"root"}
    fi
...
update_zbx_config() {
    ...
    update_config_var $ZBX_CONFIG "ListenIP" "${0.0.0.0}"
    update_config_var $ZBX_CONFIG "ListenPort" "${10051}"
    update_config_var $ZBX_CONFIG "ListenBacklog" "${ZBX_LISTENBACKLOG}"

    update_config_var $ZBX_CONFIG "SourceIP" "${90.90.90.2}"
